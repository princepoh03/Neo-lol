...
async def fetch_async(session, url, referer):
    headers = {
...
async def process_link(url):
    ...
    async with ClientSession() as session:
        for i, endpoint in enumerate(endpoints):
            ...
            content, status, error_content = await fetch_async(session, url, referer)
            if error_content:
                return {
                    "status": "error",
            ...
            for endpoint in endpoints:
                ...
                if endpoint == endpoints[-1]:
                    ...
                    if match:
                        ...
                        return jsonify({"key": match.group(1), "time_taken": time_taken, "credit": "FeliciaXxx"})
                    else:
